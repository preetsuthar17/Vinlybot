...
module.exports = {
  ...
  usage: "rex anime <anime_name>",
  run: async (client, message, args) => {
    if (!args.length)
      return message.noMentionReply(":warning: | You need provide anime name.");

    let embed = new discord.MessageEmbed()
      .setAuthor("Please wait...", client.user.displayAvatarURL())
      .setColor("#303136");
    let msg = await message.noMentionReply(embed);
    ...
    fetch("https://graphql.anilist.co", {
      ...
      .then((json) => {
        ...
        embed
          .setAuthor(
            json.title.english || json.title.romaji,
            json.coverImage.large
          )
          .setColor("#303136")(json.coverImage.color || client.settings.color)
          .setDescription(
            Replacer(json.description).substring(0, 200) +
              ` [**[Read More](${json.siteUrl})**]`
          )
        ...
        if (json.nextAiringEpisode) {
          embed.addField(
            "Episode",
            json.nextAiringEpisode.episode - 1 + "/" + (json.episodes || " --"),
            true
          );
          let time = parseMilliseconds(
            json.nextAiringEpisode.timeUntilAiring * 1000
          );
          embed.addField(
            "Next Airing",
      ...
      .catch((err) => {
        ...
        embed
          .setAuthor("Something went wrong or unable to find this anime")
          .setColor("#303136");
        return msg.edit(embed);
      });
  },
